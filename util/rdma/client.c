...
static void OnClientPreConnect(struct rdma_cm_id *id, void* ctx) {
    struct ConnectionEvent* conn_ev = (struct ConnectionEvent*)ctx;
    struct Client* client = (struct Client*)conn_ev->ctx;
    printf("client=%p, %s \n", client, __FUNCTION__);
...
static struct Client* Connect(const char* ip, const char* port, string remoteAddr) {
