...
func EpollAddSendAndRecvEvent(fd C.int, ctx unsafe.Pointer) {
	GetEpoll().EpollAdd(int(fd), func() {
		if ok := C.transport_sendAndRecv_event_cb(ctx); !ok {
			// TODO error handler
		}
	})
}
...
func ServerConnectionCallback(state C.int, cConn *C.Connection) { //TODO need to modify：感觉不是必须的
	//server := servers[cConn.agentAddr]
	server, err := rdmaConnectPool.GetRdmaServer(cConn.remoteAddr)
	if err != nil {
		//TODO error handler
	}
	//var conn *Connection
	...
	if server.conn == nil {
		...
		server.conn.cConn = cConn
		//rdmaConnectPool.PutRdmaConn(cConn.agentAddr, conn)
	}
	server.conn.state = int(state)
	//server.cb(int(state), server.conn)
}*/

/*//export ClientConnectionCallback
func ClientConnectionCallback(state C.int, cConn *C.Connection) { //TODO need to modify：感觉不是必须的
	//client := clients[cConn.agentId]
	client, err := rdmaConnectPool.GetRdmaClient(cConn.agentAddr)
	if err != nil {
		//TODO error handler
	}
	...
	if client.conn == nil {
		...
		client.conn.cConn = cConn
		//rdmaConnectPool.PutRdmaClientConn(cConn.agentAddr, conn)
	}

	client.conn.state = int(state)
	//sendBuf := &Buffer{ctx: new(proto.Packet)}
	//sendBuf.ctx.Data = []byte("hello rdma!")
	//client.cb(int(state), sendBuf, conn)
	//client.cb(int(state), client.conn)
}*/

func SendMessage() { //TODO need to modify: send data

}

//export RecvMessageCallback
func RecvMessageCallback(bufCtx *C.MemoryEntry, cConn *C.Connection) C.int { //TODO need to modify
	if cConn == nil {
		return 0
	}
	if cConn.conntype == 1 {
