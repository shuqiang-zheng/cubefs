...
func (conn *Connection) init(cConn *C.Connection) {
	...
	conn.recvMsgList = list.New()
	conn.Buffs = make(map[*byte]*Buffer)
}
	...
	if conn.recvMsgList != nil {
		for elem := conn.recvMsgList.Front(); elem != nil; elem = elem.Next() {
			...
			if msg.headerPtr != nil {
				if ret := conn.RdmaPostRecvHeader(msg.headerPtr); ret == 0 { //TODO error handler
					//return err.errors2.New("")
				}
				if msg.dataPtr != nil {
					conn.ReleaseDataBuffer(msg.dataPtr) //TODO error handler
				}
			} else {
				if msg.responsePtr != nil {
					if ret := conn.RdmaPostRecvResponse(msg.responsePtr); ret == 0 { //TODO error handler
						//return
					}
				}
			}
...
func (conn *Connection) GetDataBuffer(len uint32, timeout_us int) (*Buffer, error) {
...
func (conn *Connection) GetHeaderBuffer(timeout_us int) (*Buffer, error) { //[]byte
...
func (conn *Connection) ReleaseHeaderBuffer(headerBuff []byte) int {
	return int(C.releaseHeaderBuffer((*C.Connection)(conn.cConn), unsafe.Pointer(&headerBuff[0])))
}

func (conn *Connection) GetResponseBuffer(timeout_us int) (*Buffer, error) { //[]byte
	var bufferSize C.int32_t
	if atomic.LoadInt32(&conn.state) != CONN_ST_CONNECTED {
		return nil, fmt.Errorf("conn(%p) has been closed", conn)
	}
...
func (conn *Connection) ReleaseResponseBuffer(responseBuff []byte) int {
	return int(C.releaseResponseBuffer((*C.Connection)(conn.cConn), unsafe.Pointer(&responseBuff[0])))
}
...
func (conn *Connection) GetRecvMsgBuffer() (*Buffer, *Buffer, error) { //[]byte, []byte
	if atomic.LoadInt32(&conn.state) != CONN_ST_CONNECTED {
	...
	//return msg.headerPtr, msg.dataPtr
	return headerBuffer, dataBuffer, nil
}

func (conn *Connection) RdmaPostRecvHeader(headerBuff []byte) int {
	if atomic.LoadInt32(&conn.state) != CONN_ST_CONNECTED {
		//return fmt.Errorf("conn(%p) has been closed",conn)
		return 0
	}
	ret := int(C.rdmaPostRecvHeader((*C.Connection)(conn.cConn), unsafe.Pointer(&headerBuff[0])))
	if ret == 0 {
		//return fmt.Errorf("conn(%p) post recv header failed",conn)
		return 0
	}
	//return nil
	return 1
...
func (conn *Connection) SendResp(responseBuffer *Buffer) (int, error) { //[]byte
	...
	ret := C.connAppSendResp((*C.Connection)(conn.cConn), unsafe.Pointer(&responseBuffer.buffer[0]), C.int32_t(len(responseBuffer.buffer)))
	...
	return len(responseBuffer.buffer), nil
}

func (conn *Connection) RecvResp(*Buffer) error { //[]byte //TODO
	if atomic.LoadInt32(&conn.state) != CONN_ST_CONNECTED {
		return fmt.Errorf("conn(%p) has been closed", conn)
	}
...
func (conn *Connection) GetRecvResponseBuffer() (*Buffer, error) { //[]byte
	if atomic.LoadInt32(&conn.state) != CONN_ST_CONNECTED {
		return nil, fmt.Errorf("conn(%p) has been closed", conn)
	}
	conn.mu.RLock()
	...
	//return msg.responsePtr
	return responseBuffer, nil
}

func (conn *Connection) RdmaPostRecvResponse(responseBuff []byte) int {
	if atomic.LoadInt32(&conn.state) != CONN_ST_CONNECTED {
		//return fmt.Errorf("conn(%p) has been closed",conn)
		return 0
	}
	ret := int(C.rdmaPostRecvResponse((*C.Connection)(conn.cConn), unsafe.Pointer(&responseBuff[0])))
	if ret == 0 {
		//return fmt.Errorf("conn(%p) post recv response failed",conn)
		return 0
	}
	//return nil;
	return 1
