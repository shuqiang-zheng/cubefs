...
static int build_connection(struct rdma_cm_id *cm_id, struct ConnectionEvent *conn_ev) {
    ...
    event_ev->ctx->pd = pd; //TODO need to modify
...
static int connection_event_cb(void *ctx) {
  struct ConnectionEvent* conn_ev = (struct ConnectionEvent*)ctx;
  struct rdma_cm_event *event = NULL;
  int ret = rdma_get_cm_event(conn_ev->cm_id->ec, &event);
  if (ret < 0) {
      //TODO error handler
  ...
  if (!ret) {
      if (event->event == RDMA_CM_EVENT_ADDR_RESOLVED) { //client
          ...
          conn_ev->preconnect_callback(event->id, conn_ev); //创建connection
          ...
          struct rdma_conn_param cm_params;
          build_params(&cm_params);
          ret = rdma_accept(event->id, &cm_params);
          if (ret) {
              //TODO error handler & release resources
          ...
          //可能会有问题，需要后面再检查一下
          struct sockaddr_in *srcAddrIn = (struct sockaddr_in *)event->id->route.addr.src_addr;
          ...
          if (server->comp_channel) {
              ibv_destroy_comp_channel(server->comp_channel);
          }
          if (server->pd) {
              ibv_dealloc_pd(server->pd);
          }
          if (event->id->qp) {
              ibv_destroy_qp(event->id->qp);
          }
      ...
      if (rdma_ack_cm_event(event)) { //ack event
